...
export default function Catalog() {
  ...
  const getPageNumbers = () => {
    ...
    for (let i = Math.max(1, end - MAX_VISIBLE_PAGES + 1); i <= end; i++) {
      pageNumbers.push(i)
    }
    ...
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 pt-20 min-h-screen bg-black bg-opacity-50">
        <div className="max-w-7xl mx-auto px-4 py-8">
          <h1 className="text-5xl font-light text-center my-6 text-white">CATÁLOGO</h1>
          <p className="text-center text-lg mb-8 text-gray-200">
          ...
          <div className="mb-8 flex flex-col items-center space-y-4">
            <div className="relative w-full max-w-5xl">
              <input
                ...
                className="w-full px-6 py-4 rounded-full bg-black bg-opacity-70 text-white placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-white text-lg"
              />
              <Search className="absolute right-6 top-1/2 transform -translate-y-1/2 text-white w-6 h-6" />
            </div>
            <div className="w-full max-w-5xl overflow-x-auto">
              <div className="flex flex-nowrap justify-between min-w-full">
                {categories.map((category) => (
                  <button
                    key={category}
                    onClick={() => handleCategoryChange(category)}
                    className={`px-4 py-3 rounded-full text-base transition-colors duration-200 whitespace-nowrap ${
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center mt-8 space-x-2">
              <button
                ...
                className="p-2 rounded-full bg-black text-white hover:bg-gray-200 hover:text-black disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-black disabled:hover:text-white"
